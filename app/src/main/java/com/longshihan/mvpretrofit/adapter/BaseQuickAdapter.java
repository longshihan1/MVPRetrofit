...
import com.longshihan.mvpretrofit.animation.AlphaInAnimation;
import com.longshihan.mvpretrofit.animation.BaseAnimation;
import com.longshihan.mvpretrofit.animation.ScaleInAnimation;
import com.longshihan.mvpretrofit.animation.SlideInBottomAnimation;
import com.longshihan.mvpretrofit.animation.SlideInLeftAnimation;
import com.longshihan.mvpretrofit.animation.SlideInRightAnimation;
import com.longshihan.mvpretrofit.viewholder.BaseViewHolder1;
...
public abstract class BaseQuickAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
     ...
     * more infomation
     * see{@link  public void openLoadMore(int pageSize, boolean enable),@link  public void setOnLoadMoreListener(RequestLoadMoreListener requestLoadMoreListener)} method
     *
     * @param pageSize
     * @param requestLoadMoreListener
     */
    @Deprecated
    public void setOnLoadMoreListener(int pageSize, RequestLoadMoreListener
    ...
    /**
     * when adapter's data size than pageSize and enable is true,the loading more function is
     * enable,or disable
     *
     * @param pageSize
     * @param enable
    ...
    /**
     * call the method before you should call setPageSize() method to setting up the
     * enablePagerSize value,whether it will  invalid
    ...
    @Override
    public BaseViewHolder1 onCreateViewHolder(ViewGroup parent, int viewType) {
        BaseViewHolder1 baseViewHolder = null;
        switch (viewType) {
            case LOADING_VIEW:
                ...
                break;
            case HEADER_VIEW:
                baseViewHolder = new BaseViewHolder1(mContext, mHeaderView);
                break;
            case EMPTY_VIEW:
                baseViewHolder = new BaseViewHolder1(mContext, mEmptyView);
                break;
            case FOOTER_VIEW:
                baseViewHolder = new BaseViewHolder1(mContext, mFooterView);
                break;
            default:
    ...
    private BaseViewHolder1 getLoadingView(ViewGroup parent) {
        if (mLoadingView == null) {
            return createBaseViewHolder(parent, R.layout.def_loading);
        }
        return new BaseViewHolder1(mContext, mLoadingView);
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int positions) {
        ...
        switch (holder.getItemViewType()) {
            case 0:
                convert((BaseViewHolder1) holder, mData.get(holder.getLayoutPosition() -
                        getHeaderViewsCount()));
                addAnimation(holder);
                break;
            case LOADING_VIEW:
            ...
            default:
                convert((BaseViewHolder1) holder, mData.get(holder.getLayoutPosition() -
                        getHeaderViewsCount()));
                onBindDefViewHolder((BaseViewHolder1) holder, mData.get(holder.getLayoutPosition
                        () - getHeaderViewsCount()));
                break;
        }
    ...
    protected BaseViewHolder1 onCreateDefViewHolder(ViewGroup parent, int viewType) {
        return createBaseViewHolder(parent, mLayoutResId);
    }

    protected BaseViewHolder1 createBaseViewHolder(ViewGroup parent, int layoutResId) {
        if (mContentView == null) {
            return new BaseViewHolder1(mContext, getItemView(layoutResId, parent));
        }
        return new BaseViewHolder1(mContext, mContentView);
    }
    ...
    private void initItemClickListener(final BaseViewHolder1 baseViewHolder) {
        if (onRecyclerViewItemClickListener != null) {
            baseViewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onRecyclerViewItemClickListener.onItemClick(v, baseViewHolder
                            .getLayoutPosition() - getHeaderViewsCount());
                }
            });
        }
    ...
    private boolean isLoadMore() {
        return mNextLoadEnable && pageSize != -1 && !mLoadingMoreEnable &&
                mRequestLoadMoreListener != null && mData.size() >= pageSize;
    }
    ...
    /**
     * @see #convert(BaseViewHolder1, Object) ()
     * @deprecated This method is deprecated
     * {@link #convert(BaseViewHolder1, Object)} depending on your use case.
     */
    @Deprecated
    protected void onBindDefViewHolder(BaseViewHolder1 holder, T item) {
    }
     ...
     */
    protected abstract void convert(BaseViewHolder1 helper, T item);
